...
export const AuthGate: React.FC<AuthGateProps> = ({ children }) => {
  ...
  useEffect(() => {
    // Auto-redirect if wallet was previously connected and authenticated
    ...
    if (storedConfirmation === 'true' && authenticated && user?.wallet?.address) {
      ...
      // Redirect to /saotome if not already there
      if (pathname === '/') {
        router.push('/saotome');
      }
    }
  }, [authenticated, user, setUser, router, pathname]);
      ...
      <GlassPanel variant="default" className="max-w-md w-full p-8 text-center">
        <div className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            {authenticated ? 'Confirm your wallet to continue to application' : 'Connect your wallet to begin your citizenship application'}
          </p>
        </div>
